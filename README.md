...
1. 下载 `CodexQuotaGuard-0.2.2.exe` 放到固定可写目录，双击运行。默认注册本用户登录后自启动，不自动开启限额阻断。完整 ZIP 额外包含独立恢复工具、可选自建服务及许可文件。
...
0.2.2 包含续读改进。首次从 0.2.1 升级时没有新版续读记录，仍需在工具启动后开始新一轮任务才能建立绑定；此后满足上述条件的重启可连续采集。续读依据本机配置与日志证据，不是官方逐请求账号证明。
