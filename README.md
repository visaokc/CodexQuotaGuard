...
- 修复个人余额被限制为零的问题：已确认余额按库存减欠款显示，超额显示负数；预估消费也允许扣到负数，官方更新后校准。个人基准仍为精确的 100/3 额度点。
- 住宅 IP 功能仅显示、保留基准 IP。非基准检测地址、地区和纯净度不再进入成员界面、实时共享报告或历史记录；历史只保留变动时间。
- 保留每 10 秒检测，仅在 Codex 运行且确认 IP 不对齐时弹窗；网络检测失败不触发警报。
- 原始 Token、官方额度事实、维护开关和账号暂停设置保持不变。其他成员升级后使用相同显示和隐私规则。


## 0.6.12 更新

- IP历史只保留变动时间和“IP 已变更”，不保存旧IP、新IP、地点或纯净度。当前IP仍供实时检测和显示。
- 升级时清理本机共享账本中的旧地址，同步收到旧版记录时先去掉地址再存储。保留事实序号和原有用量、额度及维护记录。
- 其他成员需要升级到0.6.12，才会清理他们电脑上的既有历史；混用旧版可能暂时提示同步冲突。
- 当前IP不再作为历史写入磁盘，退出软件后不保留离线成员的旧地址。

## 0.6.11 更新

- 最小化、托盘或切到其他窗口时暂停界面轮询、动画和图表统计重建，回到前台立即刷新。Token采集、账本同步、官方额度查询及每10秒IP检测继续运行。
- 修复实时图表逐帧重绘导致的CPU升高，减少不可见的细微位移更新；界面快照只复制展示数据，额度算法及记账结果不变。
- 住宅IP检测改用 Ping0，每10秒检测当前出口并显示国家、地点、纯度与原始风控值。纯度数据缓存10分钟，出口改变或手动重试时重新查询。
- 住宅IP窗口改为三张并排成员卡片，IP带绿色描边和圆点，纯净度等级以绿色标明；卡片内仅列出最近20条 Codex 运行期间的IP变更，显示旧IP与新IP；首次检测、重复检测和纯净度变化不计入历史。离线成员保留最近记录及检测时间，没有上报的数据明确显示“未上报”。个人周期明细顶部新增IP卡片，与总用量并排。
- 设置/更新住宅IP基准移至设置页，概览箭头紧邻住宅IP状态。只有成功检测到IP与基准不符才显示红框；本机 Codex 正在运行时自动弹窗并从后台唤起，同一次持续异常不会重复抢焦点。网络波动、超时或检测失败只显示检测状态并自动重试，不触发红框或弹窗。
- IP历史从新版实际检测开始积累，由本人设备报告并随现有共享账本同步。不会更改Token、余额、账号暂停或维护开关。Ping0检测的是本程序出口，不保证与单独配置的 Codex 路由一致。

## 0.6.10 更新

- 暂停账号的刷新倒计时显示“已停用”，隐藏该账号的倒计时和刷新日期。
- 概览“共享组同步”卡片增加住宅IP对齐状态。管理员可将本机刚检测到、且双域名一致的出口设为共享基准；各设备独立检测，不要求所有成员在线。
- 后台每30秒检测本程序访问 chatgpt.com 与 api.openai.com 的出口；任一IP不符时显示整窗红框，并弹出“住宅IP没有对齐”，列出基准、当前IP和重试按钮。无法检测单独显示失败，成功后清除提醒。
- 此功能比较出口IP，不鉴定住宅属性，也不更改代理。Codex 单独配置的代理或按进程分流可能与检测路径不同。IP一致不能保证避免账号风控；官方说明见 https://help.openai.com/en/articles/10471992-why-am-i-receiving-a-suspicious-activity-alert 。

## 0.6.9 更新

- 设置新增账号暂停/恢复使用，由共享组管理员操作，不要求三名成员同时在线。暂停账号不计入官方可用池与个人可用余额，原始用量、历史和周期统计继续保留。
- 恢复按该账号当时实际剩余库存纳入，暂停期间刷新会正常淘汰过期库存；重复操作不会发放额外额度。个人100%仍固定为精确的100/3额度点。

## 0.6.8 更新

- 余额预估不再等待所有成员的采集确认：优先使用完整官方样本；尚无完整样本时根据已收到的用量临时估算，离线记录补齐后自动校准。
- 新周期尚无样本时，用同一账号上一周期的完整样本作为临时起点，有新周期样本后替换。预估不改写原始用量、确认账目或借还款。

## 0.6.7 更新

- 修复两个账号已确认官方临时重置、旧周期仍等待成员采集时，个人余额一直退回重置前记录的问题。旧消费及待确认历史保留；仅在旧分配不再影响当前余额时解除等待。

## 0.6.6 更新

- 官方余额和个人余额共用同一当前周期模型/缓存权重校准，持续积累已确认样本；新增用量先预估减少，官方确认后替换，避免重复扣减。显示精确到0.1个百分点，估算只用于展示，不写入借贷账本。无样本或重置待确认时保留确认值。
- 今日合计百分比与顶部官方额度采用同一基准，个人占比仍以100/3额度点为100%。今日与本周期是不同范围；估算不会改变可用账号数量或触发周期切换。
- 主界面仅变化的数位滚动，全部由上向下，旧数位下移、新数位从上方进入，动画延长至1.1秒。在线人数紧邻状态点。用户整行按个人余额从右侧以成员颜色填充，余额色块末端渐变淡入背景；超过100%时底色满格、数值保留真实余额。
- “个人用量”紧邻曲线模式，点击在右侧切换为每日个人额度消耗图；默认柱状并平滑展开，支持曲线/柱状与悬停Token详情。“软件维护模式”在设置底部，仅保留名称和开关。修复维护曲线历史滑块：切换内容保留位置、复用已缓存历史，首次加载只计算所选时间范围。
- 并发模型按各会话显示，兼容旧客户端；新端明确无活跃模型时不再回退到已结束模型。原始Token、既有归属和账本不改写。建议三端更新以获得一致的预估显示。

## 0.6.5 更新

- 个人百分比固定以精确的100/3额度点为100%，约33.33点；余额、消耗、均摊和补偿统一换算，不随可用账号数量跳变。两个账号各自刷新，未用份额随对应周期到期。
- 设置新增“软件维护模式”开关：开启者在该期间的额度和Token由三人均摊，关闭后新增用量恢复个人计入。起止记录随账本同步、重启后保留，不重复分摊；迟到的官方确认仍按实际消费时间处理。
- 饼图始终显示分摊后三人用量，不增加维护分类。普通曲线排除维护用量，选择“维护曲线”可看原始维护Token灰色曲线；原始日志与历史账本保留。
- 仅一个账号有额度时显示“单周期剩余”“单周期消耗”；两账号都有额度时保留合计和日均。新增“个人用量”查看最近30天有记录的每日个人额度百分比。
- 个人余额可依据近期已确认的同账号模型、缓存样本显示估算，鼠标提示说明，官方增量确认后校准；缺少样本保留已确认余额，估算不入借贷账本。
- 仅主界面发生变化的数位上下滑动更新；曲线与柱状图连续移动，滑块左移避免挤压图例。饼图合计移至左下并显示百分比，移除“成员已齐”，缩小启动窗口底部空白。
- 三端均需更新到0.6.5，才能使用同一固定基准和维护开关记账。发布前的封闭维护分摊不会自动变成发布后的长期分摊。

## 0.6.4 更新

- 各账号独立刷新：未用份额到期失效，已借用的份额按既有补偿规则偿还；停止0.6.3的永久未用结转，保留历史事实和规则链。
- 个人明细明确100%对应的额度点数，余额、消费、均摊和补偿统一基准。账号用尽不缩小基准；账号1旧周期排除阶段为33.33点，首次官方刷新纳入后为66.67点。
- 实时Token包含已登记均摊；旧确认余额用星号和确认时间标明，避免把不同截止范围当成同一账单。Token包含缓存，不按总Token比例直接扣额。
- 发布均摊只覆盖明确的维修起止时间，截止后新用量归实际使用者。官方确认迟到不会延长分摊范围。保留账号2完整周期、原始曲线和已确定的消费归属。三端须更新至0.6.4执行相同规则。

## 0.6.3 更新

- 官方刷新卡片的账号1整列靠左，标签、天数和日期对齐。
- 内部审查和未知模型不再单列模型卡片；周期卡片底部左侧汇总未知 Token，所有原始用量继续计入总数。
- 共享组可记录经管理员确认的未知模型平均权重分摊约定，不再因内部模型缺少公开费率而永久阻塞账本。该权重是约定的估算，官方已用总额度不变。
- 支持有明确时间范围的内部费用均摊，三位成员等份承担，成员汇总、个人明细及饼图同时均摊对应 Token；曲线和原始事件保持实记，不重复扣费。个人明细按账号展开周期，并显示“均摊消耗”卡片。三端使用0.6.3或更新版本计算同一规则。

## 0.6.2 更新

- 个人明细可切换当前配对周期、账号1/账号2的具体周期，以及封存记录；历史查看不影响当前份额。
- 成员列表依次显示在线、模型、Codex 状态、Token 和可用额度。模型与使用状态同时显示；证据未齐时显示“识别模型中”。
- 饼图默认“每天”。账号1指定旧周期首次确认已用100%后，仅饼图从该时点重新累计；旧记录留在个人明细，曲线保持完整。
- 连续90秒没有有效通信才判定离线，单次发送失败不立刻清除在线状态。恢复通信立即在线；主动移除仍立即生效。
- 自动采集共享组账号1和账号2：正常切换 Codex 登录后自动识别并纳入统计，无需手动重复扫描/添加；明确停止统计仍保留。
- 新增消费暂缺可核对模型权重时保留上次完整确认余额，以星号、时间与待分摊额度标识，当前未确认余额不冒充精确值。原始 Token 完整保留。
- 保留原有自动重置分类、账号独立发放份额及跨账号补偿规则。
...
完整算法与情形说明见 [双账号三人共享额度设计](docs/双账号三人共享额度设计.md)。

## 历史版本说明

## 0.4.8 联合样本综合估算

双方升级后，软件自动读取各自的日志并同步采集确认。只有确认之前的日志批次收齐，且双方覆盖同一段官方额度增量，才把这段合并用量纳入综合样本；按合并 Token 总量 / 对应官方额度增量推算周期容量，不平均各自的粗估值。

饼图下方显示“样本 1/2、2/2”等状态，悬停可查看样本段数、Token 和官方额度；总数据页也显示参与确认的设备数。未收齐时等待升级或同步，已有完整历史样本仍可用于评估。旧的无用量设备记录不再影响采样。历史补记或修订后会自动重新确认，重复同步不重复计数。

共同估算不依赖各自本地心跳、旧估算缓存或本地历史显示起点。两端需要运行 0.4.8 并完成同步才能一致；它仍是随模型、缓存构成变化的容量估算，不是官方固定 Token 上限。此次不更改 Token 明细与官方额度分摊算法。
...
默认显示基准为“个人额度 = 100%”；“跨周期补偿”开关默认关闭，关闭时下周期沿用平均配额。“补偿显示模式”仅切换补偿的显示。开启跨周期补偿后，后台按已确认结束周期的加权用量归属，记录实际超过个人分配的部分并补偿给有剩余额度的用户，余额按账号逐周期累计。两人各分配 50% 时，本周期个人用量 130% / 70%，下周期的补偿显示从 30% / -30% 开始，可用账号额度分别为 35% / 65%。正常刷新和已确认的重置卡重置均继承余额；空周期不清账，提前重置的未用额度不产生欠款。
...
0.4.7：图表额度只显示官方已确认增量的分摊，不外推尚未确认的消耗；Token 和已有日志明细实时累计。

图表增加“曲线模式 / 曲线＋缓存模式 / 缓存模式”下拉列表，缓存读取 Token 是输入的一部分，缓存消耗也已包含在总额度百分比中；个人显示模式按设备配额换算。旧记录缺少明细时显示“—”，缓存曲线对应时段留空，另一设备需升级后补传自身明细。一小时图表通过底部滑条查看历史，并支持回到实时。

0.4.7 数据审核修订：Token 和缓存数直接来自 Codex 用量日志，实时累计。额度百分比仅分摊官方已确认的账号增量：官方未跳动时保留上次校准值，不外推下一段消耗；从未校准的时段显示“待更新”。个人与缓存百分比是对账号消耗的分摊，不是官方个人账单。输入包含缓存命中；未命中输入 = 输入 − 缓存命中；推理包含在输出中；总 Token = 输入 + 输出。点击设备先显示本周期汇总、缓存命中率和模型明细，备注、颜色与配额操作收进“用户设置”。

采集优先读取本次请求的 `last_token_usage`，按原始时间处理累计计数回退和截断副本。历史漏记、重复或错误拆分通过带原值校验的修订记录更正，保留原始同步事实；反复扫描不会重复补记。已知事件可补齐输入、缓存、输出和可用的推理明细；无法确认账号归属的记录仍不强行计入。双方都需要升级，才能一致显示修订后的历史与缓存。

缓存曲线用虚线；“曲线＋缓存模式”切换为柱状时只绘制总 Token 柱形。小时视图通过左下角短滑条选择一个小时，可向前回看 24 小时；一天视图按整小时选择，最多向前回看 30 天。滑条按所选用户和模型的最早记录收紧范围，无历史记录时禁用，取消直接拖动图形；预加载完整回看范围及相邻小时，按整分钟或两分钟记数，以持续运行的逐帧动画跟随滑动目标，连续拖动不会反复取消动画；滑块与动画位置独立，官方跳动后刷新历史校准。纵轴随当前窗口用量平滑放大或缩小，滑到最右端连续恢复实时、不重播入场动画；点击“回到实时”按钮仍重播绘制动画。快速切换清除旧悬浮标记，绘图裁剪在 SVG 内。浮窗将 Token 实记值与官方额度分摊分列，缺失明细明确标注。图表、设备列表与用户汇总统一使用按时段分摊的额度显示。用户汇总中的模型统计放在总用量下方，每行最多四张紧凑卡片，保留模型 Token、占比、缓存命中、未命中输入、输出及命中率。

“跨周期补偿”开关默认关闭、按账号同步。关闭时各周期沿用原始配额（两人各 50%），不继承超额；个人已用百分比可超过 100%。重新开启后不追补关闭期间已经结束的周期，开启所在周期结束后才恢复结算。旧版客户端不识别开关，双方需升级才能执行相同规则。用户模型进度条加粗，并显示该模型 Token 占该用户本周期总 Token 的比例；缓存命中率独立保留在明细中。astra、sol、terra、luna 固定显示，未用模型显示 0 和 0%，不附加未使用提示；实际使用的其他模型仍保留。总数据模型卡片同样显示该周期总 Token 占比和 4px 细进度条，上方总额度进度条为 8px。

取消“本机估算用量已达到配额”的弹出提示。

设备行在线/离线标签使用固定列，与使用状态、Token、个人百分比对齐；在线底色、字体和圆点为绿色，个人百分比放大。按最终要求不展示或同步当前模型和思考程度，使用状态保留单行显示。用户模型卡片的标题为 12px、Token 数字为 16px、明细为 11px 并保留 16px 行高。模型筛选顺序为 astra、sol、terra、luna。
