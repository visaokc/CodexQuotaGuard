...
- 住宅IP窗口改为三张并排成员卡片，IP带绿色描边和圆点，纯净度等级以绿色标明；卡片内可展开最近20条变化记录。离线成员保留最近记录及检测时间，没有上报的数据明确显示“未上报”。个人周期明细底部新增该成员的IP明细。
- 设置/更新住宅IP基准移至设置页，概览箭头紧邻住宅IP状态。IP不符继续显示红框；只有检测到本机 Codex 正在运行才自动弹窗，后台时唤起主窗口，同一次持续异常不会重复抢焦点。
