...
$dist = Join-Path $PSScriptRoot 'dist\0.3.2'
...
& $python -m PyInstaller --noconfirm --clean --onefile --windowed --name 'Codex配额管家' --icon assets\app.ico --collect-all customtkinter --hidden-import pystray._win32 --exclude-module quota_guard.mesh --exclude-module quota_guard.file_mesh --exclude-module aiortc --exclude-module av --exclude-module pylibsrtp --exclude-module PIL._avif --exclude-module PIL._webp --exclude-module PIL._imagingft --add-binary 'vendor\tsnet\cqg-tsnet.exe;vendor\tsnet' --add-binary 'work\updater\CodexQuotaUpdater.exe;vendor' --add-data 'licenses;licenses' --add-data 'vendor\tsnet\manifest.json;vendor\tsnet' --add-data 'vendor\tsnet\LICENSE.txt;licenses\tsnet' --add-data 'vendor\tsnet\THIRD_PARTY_LICENSES.txt;licenses\tsnet' --distpath $dist --workpath work\build-client main.py
