  ...
  <div class="app-shell" :class="{ready}">
    ...
    <main><Transition name="page" mode="out-in">
      <section v-if="page==='overview'" key="overview" class="overview page">
        ...
        <div class="charts-grid"><article class="chart-card donut-card" data-testid="pie-chart"><h2>设备使用占比</h2><DonutChart :devices="devices" :totals="pie.totals" :quota-total="cycleBudget.total_tokens"/><div class="cycle-budget" data-testid="cycle-budget" :title="'按账号官方额度与同步 Token 比例推算；来源：'+(cycleBudget.source||'等待样本')+'。所有模型的本周期估计，不随图表筛选改变。'"><span>推算本周期总量</span><strong>{{cycleBudget.total_tokens>0?'≈ '+compact(cycleBudget.total_tokens):'等待样本'}} <small v-if="cycleBudget.total_tokens>0">Token</small></strong></div><div class="pie-period"><SelectBox v-model="piePeriod" :options="[{value:'cycle',label:'本周期'},{value:'pie_hour',label:'近一小时'},{value:'pie_six_hours',label:'近六小时'},{value:'today',label:'今天'},{value:'week',label:'近7天'},{value:'month',label:'近30天'},{value:'total',label:'累计使用'}]" label="设备占比时间范围"/></div></article><article class="chart-card trend-card" data-testid="trend-chart"><h2>{{trendName}}<span data-testid="trend-total">{{compact(trend.total)}} Token</span></h2><TrendChart :quota-total="cycleBudget.total_tokens" :data="trend" :kind="kind" :color="trendColor"/><div class="chart-footnote">原始 Token 用量 · 悬停查看数值</div></article></div>
