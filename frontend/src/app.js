...
import {historyWindow,historyMinimum,chartQuotaPercent,userBreakdown,cycleModels,COLORS,COLOR_NAMES,compact,devicesFor,aggregate,trendForMode,timeLabel,modelOptions,officialUsageColor,officialQuotaRemaining,refreshRemaining,dailyQuotaUsage} from './data.js';
...
const app=createApp({
  ...
  setup(){
    ...
    const model=ref(''),user=ref(''),kind=ref('line'),curveMode=ref('total'),period=ref('day'),piePeriod=ref('cycle');
    ...
    const selected=computed(()=>devices.value.find(d=>d.id===selectedDevice.value)),userCycle=computed(()=>userBreakdown(snapshot.value,selectedDevice.value));
    ...
    function panChart(end){
      ...
      const latest=view.value.analytics?.at||Date.now()/1000,step=period.value==='day'?3600:60,requested=Math.max(historyMin.value,end),target=requested>=Math.floor(latest/step)*step?latest:requested;
    ...
    async function loadChartHistory(end){
      ...
      try{while(pendingHour!==null){
        const end=pendingHour,request=historyRequest,selected=account.value,selectedPeriod=period.value;pendingHour=null;
        try{
          const response=await bridge()?.command('chart_history',{account:selected,end,period:selectedPeriod});
          if(request!==historyRequest||selected!==account.value)continue;
    ...
    watch([period,account],()=>{liveChart();chartHistory.value=null;if(pannable.value&&account.value)loadChartHistory(view.value.analytics?.at||Date.now()/1000);});
    watch(()=>[epoch.value?.used,epoch.value?.reset_at].join(':'),()=>{if(pannable.value&&account.value)loadChartHistory(view.value.analytics?.at||Date.now()/1000);});
    watch(historyMin,value=>{if(chartEnd.value!==null&&(chartTarget.value??chartEnd.value)<value){const step=period.value==='day'?3600:60;if(value>=Math.floor(view.value.analytics.at/step)*step)liveChart(false);else panChart(value);}});
    ...
    const trendName=computed(()=>chartEnd.value!==null&&pannable.value?(period.value==='day'?'历史 24 小时':'历史 1 小时'):({hour:'最近 1 小时',day:'最近 24 小时',week:'最近 7 天',month:'最近 30 天'})[period.value]);
    ...
    function applySnapshot(data){
      ...
      if(oldAccount!==account.value){lastReceipt.value=null;selectedDevice.value='';model.value='';user.value='';activityUntil.clear();cancelDeviceDrag();}
    ...
    function deviceUsing(d){return deviceConnected(d)&&d.online&&(d.active>0||(activityUntil.get(d.id)||0)>clockNow.value);}


    function deviceState(d){return !deviceConnected(d)?'离线 / 已切换':d.online?(deviceUsing(d)?'Codex 使用中':d.unbound_active?'活动待归属':'暂无近期活动'):'连接在线 · 状态过期';}
    function usage(d){const value=aggregate(snapshot.value,'cycle','',d.id),cap=d.fair_base_cap??d.cap,carry=view.value.summary?.compensation_enabled&&settings.value.quota_display==='fair'&&cap>0?(d.carry||0)/cap*100:0;return chartQuotaPercent((value.quotaTotals[d.id]||0)+carry,value.quotaReady,value.quotaStates[d.id]==='pending',value.quotaStates[d.id]==='estimated');}
  ...
  <div class="app-shell" :class="{ready}">
    ...
    <main><Transition name="page" mode="out-in">
      <section v-if="page==='overview'" key="overview" class="overview page">
        ...
        <div class="chart-controls"><div class="filters"><SelectBox v-model="model" :options="models" label="筛选模型"/><SelectBox v-model="user" :options="users" label="筛选用户"/></div><div class="chart-modes"><SelectBox v-model="curveMode" :options="[{value:'total',label:'曲线模式'},{value:'combined',label:'曲线＋缓存模式'},{value:'cache',label:'缓存模式'}]" label="曲线内容模式"/><div class="segment"><button :class="{active:kind==='line'}" @click="kind='line'">曲线</button><button :class="{active:kind==='bar'}" @click="kind='bar'">柱状</button></div><SelectBox v-model="period" :options="[{value:'hour',label:'一小时'},{value:'day',label:'一天'},{value:'week',label:'一周'},{value:'month',label:'一月'}]" label="趋势时间范围"/></div></div>
        <div class="charts-grid"><article class="chart-card donut-card" data-testid="pie-chart"><h2>设备使用占比</h2><DonutChart :devices="devices" :totals="pie.totals" :quota-totals="pie.quotaTotals" :quota-ready="pie.quotaReady" :quota-states="pie.quotaStates" :cache-totals="pie.cacheQuotaTotals" :cache-tokens="pie.cacheTotals" :cache-missing="pie.cacheMissingTotals"/><div class="cycle-budget" data-testid="cycle-budget" :title="cycleBudget.sample_devices!==undefined?'双方采集确认后按同一时段合并计算：'+(cycleBudget.sample_ready||0)+'/'+cycleBudget.sample_devices+' 台已确认，'+(cycleBudget.sample_segments||0)+' 段样本，'+compact(cycleBudget.sample_tokens)+' Token / '+(cycleBudget.sample_percent?.toFixed(1)||'0')+'% 官方额度。样本尚未收齐时等待升级或同步。':'按账号官方额度与同步 Token 比例推算；来源：'+(cycleBudget.source||'等待样本')"><span>推算本周期总量<template v-if="cycleBudget.sample_devices!==undefined"> · 样本 {{cycleBudget.sample_ready}}/{{cycleBudget.sample_devices}}</template></span><strong>{{cycleBudget.total_tokens>0?'≈ '+compact(cycleBudget.total_tokens):cycleBudget.sample_devices!==undefined?'等待样本同步':'等待样本'}} <small v-if="cycleBudget.total_tokens>0">Token</small></strong></div><div class="pie-period"><SelectBox v-model="piePeriod" :options="[{value:'cycle',label:'本周期'},{value:'pie_hour',label:'近一小时'},{value:'pie_six_hours',label:'近六小时'},{value:'today',label:'今天'},{value:'week',label:'近7天'},{value:'month',label:'近30天'},{value:'total',label:'累计使用'}]" label="设备占比时间范围"/></div></article><article class="chart-card trend-card" data-testid="trend-chart"><h2>{{trendName}}<button v-if="chartEnd!==null" class="live-hour" @click="liveChart">回到实时</button><span data-testid="trend-total">{{compact(trend.total)}} Token</span></h2><TrendChart :data="trend" :kind="kind" :color="trendColor" :pannable="pannable" :earliest-end="historyMin" :slider-step="period==='day'?3600:60" :slider-label="period==='day'?'最近30天时间滑块':'最近24小时时间滑块'" :view-end="chartEnd??view.analytics?.at" :slider-end="chartTarget??view.analytics?.at" :latest-end="view.analytics?.at" :live-revision="liveRevision" @pan="panChart"/><div class="chart-footnote">{{curveMode==='total'?'原始 Token 用量':curveMode==='combined'?(kind==='bar'?'总 Token 柱状':'实线：总 Token · 虚线：缓存 Token'):'缓存 Token · 缺失明细的时段留空'}} · 额度随官方更新校准</div></article></div>
