...
import {COLORS,COLOR_NAMES,compact,devicesFor,aggregate,quotaValue,timeLabel,modelOptions,officialUsageColor,officialQuotaRemaining,refreshRemaining} from './data.js';
...
const app=createApp({
  ...
  setup(){
    ...
    const trendName=computed(()=>({day:'最近 24 小时',week:'最近 7 天',month:'最近 30 天'})[period.value]);
    ...
    const nav=[['overview','概览','grid'],['accounts','账号','user'],['sync','同步','sync'],['settings','设置','settings'],['help','说明','info']];
    ...
    function applySnapshot(data){
      ...
      if(!devices.value.some(d=>d.id===user.value))user.value='';
    ...
    watch(()=>modal.value,()=>{if(!modal.value){clearTimeout(pairTimer);pairPending.value=false;}});
  ...
  <div class="app-shell" :class="{ready}">
    <header class="titlebar" @mousedown="nativeDrag" @dblclick.self="host('maximize')">
      ...
      <nav><button v-for="[id,label,icon] in nav" :key="id" :data-testid="'nav-'+id" :class="{active:page===id}" @click="switchPage(id)"><svg viewBox="0 0 20 20" aria-hidden="true"><g v-if="icon==='grid'"><rect x="3" y="3" width="5" height="5" rx="1"/><rect x="12" y="3" width="5" height="5" rx="1"/><rect x="3" y="12" width="5" height="5" rx="1"/><rect x="12" y="12" width="5" height="5" rx="1"/></g><g v-else-if="icon==='user'"><circle cx="10" cy="6" r="3"/><path d="M4 17v-2a6 6 0 0 1 12 0v2"/></g><g v-else-if="icon==='sync'"><path d="M3 7h13l-3-3m4 9H4l3 3"/></g><g v-else-if="icon==='settings'"><path d="m8 2 4 0 1 3 3 1 2 4-2 4-3 1-1 3H8l-1-3-3-1-2-4 2-4 3-1z"/><circle cx="10" cy="10" r="3"/></g><g v-else><circle cx="10" cy="10" r="7"/><path d="M10 9v5M10 5v1"/></g></svg><span>{{label}}</span></button></nav>
      <span class="version">v{{snapshot.version||'…'}}</span><div class="window-buttons"><button @click="host('minimize')" title="最小化" aria-label="最小化"><svg viewBox="0 0 16 16"><path d="M4 8h8"/></svg></button><button class="window-close" @click="host('close')" title="隐藏到托盘" aria-label="隐藏到托盘"><svg viewBox="0 0 16 16"><path d="m5 5 6 6m0-6-6 6"/></svg></button></div>
    </header>
    <main><Transition name="page" mode="out-in">
      <section v-if="page==='overview'" key="overview" class="overview page">
        <div class="summary-grid">
          <article class="summary-card"><span class="eyebrow">官方周额度剩余</span><strong data-testid="official-remaining" :style="{color:officialUsageColor(officialUsed)}">{{officialLeft!==null?officialLeft.toFixed(0)+'%':'—'}}</strong><div class="quota-progress" role="progressbar" :aria-valuenow="officialLeft" aria-valuemin="0" aria-valuemax="100"><i :style="{transform:'scaleX('+(officialLeft||0)/100+')',background:officialUsageColor(officialUsed)}"></i></div></article>
