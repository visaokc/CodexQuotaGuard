export {PersonalUsageChart} from './personal-usage.js';
...
const RollingDigit={
  props:{value:String},
  setup(props){
    const previous=ref(''),changing=ref(false),direction=ref(1),revision=ref(0);let timer=0;
    watch(()=>props.value,(value,old)=>{
      clearTimeout(timer);changing.value=false;
      value=String(value??'');old=String(old??'');
      if(!/^[0-9]$/.test(value)||!/^[0-9]$/.test(old))return;
      previous.value=old;direction.value=-1;revision.value++;
      nextTick(()=>{changing.value=true;timer=setTimeout(()=>changing.value=false,1100);});
    });
    onBeforeUnmount(()=>clearTimeout(timer));
    return {previous,changing,direction,revision};
  },
  template:`<span class="rolling-digit" :class="{'number-changing':changing}" :data-previous="previous" :style="{'--roll-direction':direction}"><span class="rolling-digit-current" :key="revision">{{value}}</span></span>`
};
