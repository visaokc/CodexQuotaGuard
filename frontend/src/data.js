...
export function aggregate(snapshot, window, model='', device='') {
  ...
  const points=Array(Math.max(1,source.count||1)).fill(0), totals={},quotaPoints=points.slice(),quotaTotals={},quotaPendingPoints=points.map(()=>false);
  ...
  return {quotaPendingPoints,quotaPoints,quotaTotals,quotaReady:source.quota_ready===true,points,series,totals,total:Object.values(totals).reduce((a,b)=>a+b,0),start:source.start||0,step:source.step||1};
}
