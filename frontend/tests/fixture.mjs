...
export function fixture(){
  ...
  const windows={};
  for(const [name,seconds,count] of [['cycle',604800,1],['hour',3600,60],['day',86400,24],['week',604800,7],['month',2592000,30],['total',at,1]]){
    const rows=[];
    ...
    const step=seconds/count,start=['hour','day','week','month'].includes(name)?Math.floor(at/step)*step-(count-1)*step:at-seconds;
    windows[name]={count,start,step,rows};
  }
