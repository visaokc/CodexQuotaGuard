...
try{
  ...
  await page.addInitScript(data=>{window.__fixture=data;window.__commands=[];window.__CQG_TEST_BRIDGE__={snapshot:async()=>{if(!window.__hasSnapshot){window.__hasSnapshot=true;await new Promise(resolve=>setTimeout(resolve,150));}return structuredClone(window.__fixture);},command:async(action,payload)=>{window.__commands.push({action,payload});if(action==='member_history'){await new Promise(r=>setTimeout(r,payload.cycle==='archive'?120:15));const data=structuredClone(window.__fixture.view.analytics);data.windows.cycle=structuredClone(window.__memberWindows?.[payload.cycle]||data.windows.cycle);return{ok:true,data};}if(action==='chart_history'){await new Promise(r=>setTimeout(r,window.__historyDelay||0));return{ok:true,data:structuredClone(window.__history||window.__fixture.view.analytics)};}return {ok:true,data:{}};},window_action:async()=>({ok:true})};},billingFixture());
  ...
  await page.waitForFunction(()=>{const n=document.querySelector('.hour-slider input');return n&&!n.disabled;});
  ...
  const priorEnd=(await page.evaluate(()=>window.__CQG_TEST__.getState())).chartEnd;
  await page.evaluate(()=>{window.__fixture.view.analytics.at+=120;window.__historyDelay=1000;window.__CQG_TEST__.applySnapshot(structuredClone(window.__fixture));});
  const cachedCalls=await page.evaluate(()=>window.__commands.filter(c=>c.action==='chart_history').length);
  await slider.evaluate(node=>{node.value=String(Number(node.value)-180);node.dispatchEvent(new Event('input',{bubbles:true}));});
  await page.waitForTimeout(80);
  assert.ok((await page.evaluate(()=>window.__CQG_TEST__.getState())).chartEnd<priorEnd,'stale live edge does not delay a cached historical seek');
  assert.equal(await page.evaluate(()=>window.__commands.filter(c=>c.action==='chart_history').length),cachedCalls,'cached target never waits for a fresh live edge');
  await page.waitForTimeout(350);const keptEnd=(await page.evaluate(()=>window.__CQG_TEST__.getState())).chartEnd;
  await page.getByRole('button',{name:'曲线内容模式',exact:true}).click();await page.getByRole('option',{name:'曲线模式',exact:true}).click();
  assert.equal((await page.evaluate(()=>window.__CQG_TEST__.getState())).chartEnd,keptEnd,'changing curve content preserves selected history');assert.equal(await slider.isEnabled(),true);
  assert.equal(await page.evaluate(()=>window.__commands.filter(c=>c.action==='chart_history').length),cachedCalls,'content mode switches reuse the same history');
  await page.getByRole('button',{name:'曲线内容模式',exact:true}).click();await page.getByRole('option',{name:'维护曲线',exact:true}).click();
