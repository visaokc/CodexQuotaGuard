...
def presentation(flow, view, now=None):
    ...
    elif view.get('identity', {}).get('mode') in ('api', 'none', 'logged_out') or view.get('pair_scope') is False:
        title, detail, color = '当前未同步 · 请登录已添加的订阅账号', '配对信息仍保留。API 或未添加账号不会启动订阅用量同步。', '#f2b46f'
        steps[1:] = [False]*3
    elif recent:
        title = f'已连接 {len(peers)} 台设备 · 已收到同步数据'
        detail = f'已收到并处理 {len(recent)}/{len(peers)} 台设备的近期同步消息；不表示全部历史记录已补传完成。'
    elif peers and any(p in receipts for p in peers):
        title, detail, color = f'已连接 {len(peers)} 台设备 · 同步消息暂未更新', '之前已收到同步，但近期没有新回执；正在等待下一次同步，尚不能确认数据是否最新。无需重复粘贴。', '#f2b46f'
    elif peers:
        title, detail = f'已连接 {len(peers)} 台设备 · 等待首次用量同步', '同账号设备已通过加密校验，尚未收到可确认的用量同步。无需重复粘贴。'
    elif receipts:
        title, detail, color = '设备连接已中断 · 正在自动重连', '保留已保存的配对信息，不需要重新粘贴。尚不能确定是对方退出、账号切换还是网络中断。', '#f2b46f'
    elif stage == 'ready':
        title = '匹配码已生成 · 请复制给对方'
        detail = '对方粘贴并确认后自动连接。尚未发现同账号设备，无法判断对方是否已粘贴或是否在线。'
    elif stage == 'saved':
        title = '配对信息已保存 · 正在连接'
        detail = '确认后配置立即保存，连接与同步仍需等待，无需再次粘贴。双方须运行本工具并登录同一个已添加账号。'
    else:
...
class PairPanel(ctk.CTkFrame):
    ...
    def render(self, flow, view):
        ...
        if flow.get('stage') == 'saved' and not self.code:
            ...
            self.copy.pack(side='left')
        for label, complete in zip(self.steps, value['steps']):
            label.configure(fg_color='#203e35' if complete else '#263549', text_color='#69d9bd' if complete else '#8c9eb6')
