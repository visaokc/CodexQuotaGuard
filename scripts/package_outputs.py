...
dist = root/'dist'/'0.6.1'
with zipfile.ZipFile(out/'CodexQuotaGuard-0.6.1-Windows.zip', 'w', zipfile.ZIP_DEFLATED) as z:
...
with zipfile.ZipFile(out/'CodexQuotaGuard-0.6.1-Source.zip', 'w', zipfile.ZIP_DEFLATED) as z:
...
shutil.copy2(dist/'Codex配额管家.exe', out/'CodexQuotaGuard-0.6.1.exe')
shutil.copy2(dist/'恢复Codex网络.exe', out/'CodexQuotaRecovery-0.6.1.exe')
for name in ('Codex配额管家.exe', '恢复Codex网络.exe'):
    shutil.copy2(dist/name, out/(Path(name).stem+'-0.6.1.exe'))
shutil.copy2(root/'README.md', out/'使用说明-0.6.1.md')
shutil.copy2(root/'VALIDATION.md', out/'验证记录-0.6.1.md')
preview = root/'frontend'/'test-artifacts'/'overview.png'
if preview.exists():
    shutil.copy2(preview, out/'CodexQuotaGuard-UI-0.6.1.png')
names = ('CodexQuotaGuard-0.6.1.exe', 'CodexQuotaGuard-0.6.1-Windows.zip', 'CodexQuotaGuard-0.6.1-Source.zip', 'CodexQuotaRecovery-0.6.1.exe')
lines = []
for name in names:
    path = out/name
...
(out/'SHA256SUMS-0.6.1.txt').write_text('\n'.join(lines)+'\n', encoding='utf-8')
assert hashlib.sha256((out/'Codex配额管家-0.6.1.exe').read_bytes()).digest() == hashlib.sha256((dist/'Codex配额管家.exe').read_bytes()).digest()
